    ...
    <v-card>
        ...
        <v-card-title>
            <v-toolbar-title class="text-capitalize">
                ...
                {{ $tc('lang.action.create') }} {{ $tc('lang.module.role', 1) }}
            </v-toolbar-title>
                    ...
                    <v-layout wrap>
                        <v-flex xs12 md5 d-flex>
                            <v-text-field v-validate="'required|min:3|max:100'" :counter="100" 
                                :error-messages="errors.collect('name')" data-vv-name="name" name="name" 
                                :label="$t('lang.forms.name')" type="text" :value="item.name" @input="updateName">
                            </v-text-field>
                        </v-flex>
                        <v-flex xs12 md5 d-flex>
                            <v-text-field name="slug" :label="$t('lang.forms.slug')" 
                                disabled="" type="text" :value="item.slug">
                            </v-text-field>
                        </v-flex>
                        <v-flex xs12 md2 d-flex>
                            <v-text-field v-validate="'required|between:1,5'" :error-messages="errors.collect('level')" 
                                data-vv-name="level" name="level" :label="$t('lang.forms.level')" type="number" min="1" max="5" :value="item.level" @input="updateLevel">
                            </v-text-field>
                        </v-flex>
                        <v-flex xs12 md12 d-flex>
                            <v-textarea v-validate="'required|max:255'" :counter="255" :error-messages="errors.collect('description')" 
                                data-vv-name="description" name="description" :label="$t('lang.forms.description')" type="text" :value="item.description" @input="updateDescription">
                            </v-textarea>
                        </v-flex>
                    </v-layout>
                </v-container>
...
export default {
    ...
    methods: {
        ...mapActions('RolesSingle', ['storeData', 'setName', 'setLevel', 'setDescription', 'resetState']),
        validate () {
